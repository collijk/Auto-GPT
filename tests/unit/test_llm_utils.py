...
from autogpt.llm.llm_utils import create_chat_completion, get_ada_embedding
...
def mock_create_embedding(mocker, prompt_tokens):
    ...
    return mocker.patch(
        "autogpt.llm.llm_utils.openai_provider.create_embedding",
...
def mock_create_chat_completion(mocker, prompt_tokens, completion_tokens):
    ...
    return mocker.patch(
        "autogpt.llm.llm_utils.openai_provider.create_chat_completion",
        return_value=mock_response,
    )
...
def test_get_ada_embedding(
    mock_create_embedding, budget_manager, embedding_model, prompt_tokens
):
    arg = "test"
    ...
    assert budget_manager.total_prompt_tokens == prompt_tokens
    assert budget_manager.total_completion_tokens == 0
    assert budget_manager.total_cost == (prompt_tokens * cost) / 1000
